...
std::vector<unsigned char> uniform_real_distribution_stream(){
    std::vector<unsigned char> random_stream;
    std::default_random_engine generator;
    std::uniform_real_distribution<double> distribution(0.0,1.0);

    for(int i = 0; i < 32772; ++i){
        union{
            double generated;
            unsigned char generatedChars[sizeof(double)];
        } generatedUnion;
        generatedUnion.generated = distribution(generator);
        for(int j=0; j < sizeof(double); j++){
        random_stream.push_back(generatedUnion.generatedChars[j]);
        }
    }
...
std::vector<unsigned char> normal_distribution_stream(){
    ...
    for(int i = 0; i < 32772; ++i){
        ...
        generatedUnion.generated = dist(generator);
        for(int j=0; j < sizeof(double); j++){
        random_stream.push_back(generatedUnion.generatedChars[j]);
        }
    }
...
std::vector<unsigned char> exponential_distribution_stream(){
    std::vector<unsigned char> random_stream;
    static std::default_random_engine generator;
    static std::exponential_distribution<double> dist(3.5);
...
int main()
{
    std::vector<unsigned char> normalized_bytes = normal_distribution_stream();
    std::vector<unsigned char> uniformed_bytes = uniform_real_distribution_stream();
    std::vector<unsigned char> exponential_bytes = exponential_distribution_stream();
    std::cout << "Test dla ciągu o rozkładzie równomiernym" << std::endl;
    test_file(uniformed_bytes);
    std::cout << "------------------------------------" << std::endl;
    std::cout << "Test dla ciągu o rozkładzie normalnym" << std::endl;
    test_file(normalized_bytes);
    std::cout << "------------------------------------" << std::endl;
    std::cout << "Test dla ciągu o rozkładzie Laplace" << std::endl;
    test_file(exponential_bytes);
    std::cout << "------------------------------------" << std::endl;
    std::cout << "Testy dla obrazów testówych" << std::endl;
    test_files();
    return 0;
}
